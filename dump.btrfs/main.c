...
static const char * const cmd_inspect_dump_tree_usage[] = {
	"dump.btrfs [options] <device> [<device> ..]",
...
static void print_usage(FILE *out)
{
	...
	for (line = cmd_inspect_dump_tree_usage; *line; line++) {
		const char *text = *line;

		if (text[0] == HELPINFO_OPTION[0]) {
			const char *desc = strchr(text + 1, HELPINFO_DESC[0]);

			if (desc) {
				fprintf(out, "  %-28.*s %s\n",
					(int)(desc - text - 1), text + 1,
					desc + 1);
			} else {
				fprintf(out, "  %s\n", text + 1);
			}
...
static int dump_tree_main(int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "hdeb:rRut:", long_options, NULL);
		if (c < 0)
			break;
		switch (c) {
		case 'h':
...
{
	return dump_tree_main(argc, argv);
}
